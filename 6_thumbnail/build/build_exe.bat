...
echo 빌드 도구·모듈 의존성 설치...
"!PYEXE!" -m pip install -q -r "%~dp0requirements-build.txt"
if errorlevel 1 exit /b 1
"!PYEXE!" -m pip install -q -r "%~dp0..\requirements.txt"
if errorlevel 1 exit /b 1

echo PyInstaller 실행...
"!PYEXE!" -m PyInstaller --clean --noconfirm --distpath "!PROOT!\dist" --workpath "%~dp0work" "%~dp0thumbnail_gui.spec"
if errorlevel 1 exit /b 1

echo.
echo 완료: "!PROOT!\dist\ThumbnailStudio.exe"
exit /b 0
