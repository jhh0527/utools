#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GUI 진입점 — ``dist/2_2_imgDown_gui.exe`` 우선."""
...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "2_2_imgDown_gui.exe"
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        try:
            from img_down.gui_app import main as gui_main
    ...
    exe = _dist_gui_exe()
    use_source = os.environ.get("IMG_DOWN_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _show_error_dialog() -> None:
    try:
        ...
        r = tk.Tk()
        r.withdraw()
        messagebox.showerror("2_2 imgDown", traceback.format_exc())
        r.destroy()
    except Exception:
        pass
